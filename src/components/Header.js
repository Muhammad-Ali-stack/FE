  ...
  return (
    <nav style={{ backgroundColor: "#9B0020" }} className="p-4 shadow-md">
      <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
        ...
        {/* Navbar Links */}
        <div className={`md:flex space-x-8 ${menuOpen ? "block" : "hidden"} md:block mb-4 md:mb-0`}>
          <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-8">
            <a href="/" className="relative text-white hover:text-white group">
              Home
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
            </a>
            <a href="#about" className="relative text-white group hover:text-white">
              About Us
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
            </a>
            {auth?.user && (
              <>
                <a href="/all-conferences" className="relative text-white group hover:text-white">
                  CFP
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
                </a>
                <a href="/create-conference" className="relative text-white group hover:text-white">
                  Create Conference
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
                </a>
              </>
        ...
        {!auth?.user ? (
          <div className="space-x-4 mb-4 md:mb-0">
            <a
              href="/register"
              className="text-white px-5 py-2 rounded-md font-medium transition-all duration-300 hover:bg-white hover:text-red-900"
                ...
                <div className="absolute right-0 z-10 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div className="py-1" role="menu" aria-orientation="vertical">
                    <ul>
                      <li>
                        <a
                          href={`/${auth?.user?.role === 1 ? "admindashboard/admin-dashboard" : "userdashboard/user-dashboard"}`}
